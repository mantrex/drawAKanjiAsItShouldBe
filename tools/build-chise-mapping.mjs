// build-chise-mapping.mjs
//
// Phase 4 (consolidation): merges Phase 2's deterministic matches
// (chisedata/chise-kanjivg-match-pass1.json — resolved-direct /
// resolved-variant / unresolved) with Phase 3's per-component judgments
// (chisedata/chise-phase3-components-resolved.json) into a single,
// complete mapping: chisedata/chise-kanjivg-mapping-final.json.
//
...
//
// Usage: node tools/build-chise-mapping.mjs
// Writes: chisedata/chise-kanjivg-mapping-final.json,
//         chisedata/chise-final-mapping-log.md
...
say(`# Final CHISE ↔ KanjiVG mapping — consolidation log\n`);
say(`Run at: ${new Date().toISOString()}\n`);

const pass1 = JSON.parse(readFileSync(join(OUT_DIR, "chise-kanjivg-match-pass1.json"), "utf8"));
const phase3 = JSON.parse(readFileSync(join(OUT_DIR, "chise-phase3-components-resolved.json"), "utf8"));
...
writeFileSync(
  join(OUT_DIR, "chise-kanjivg-mapping-final.json"),
  JSON.stringify(
    {
      generatedAt: new Date().toISOString(),
      description:
        "Complete CHISE/IDS-to-KanjiVG component mapping. Each entry is one (kanji, CHISE component) pair with its final resolution: 'direct' (exact string match), 'variant' (matched via KanjiVG's own kvg:original attribute), 'agent-semantic' (matched by AI judgment in Phase 3, see note), 'absent' (CHISE component genuinely has no corresponding named node in this kanji's KanjiVG file), 'structural-mismatch' (CHISE and KanjiVG segment this kanji fundamentally differently, see note), or 'no-kanjivg-file' (no KanjiVG SVG exists for this kanji at all). This file is derived from GPLv2-licensed CHISE/IDS data — never bundled with the DAKAISB npm package.",
      sourceAttribution:
...
say(`\nWrote ${join(OUT_DIR, "chise-kanjivg-mapping-final.json")} (${finalEntries.length} entries).`);

writeFileSync(join(OUT_DIR, "chise-final-mapping-log.md"), log.join("\n") + "\n");
say(`Wrote ${join(OUT_DIR, "chise-final-mapping-log.md")}.`);

// prepare-chise-submax-phase3.mjs
//
// Same purpose as tools/prepare-chise-phase3.mjs (CHISE_MAIN's own Phase 3
// prep script), adapted for CHISE_SUBMAX's pipeline: groups
// chisedata/chise-submax-kanjivg-match-pass1.json's unresolved (kanji,
// CHISE-component) pairs by distinct component (not per pair — same ~N-fold
// reduction in judgment work already proven for KRAD and CHISE_MAIN), and
// prepares a candidate-elements record per component for an agent to judge.
// Component values here are CHISE_SUBMAX's own true-IDS-leaf components,
// not CHISE_MAIN's KanjiVG-gated ones — the two are judged as entirely
// separate component sets, even where a component string happens to
// coincide between them.
//
// Usage: node tools/prepare-chise-submax-phase3.mjs
// Writes: chisedata/chise-submax-phase3-components.json (input for agent judgment)
//         chisedata/chise-submax-phase3-prep-log.md
...
say(`# CHISE_SUBMAX Phase 3 preparation log\n`);
...
writeFileSync(
  join(OUT_DIR, "chise-submax-phase3-components.json"),
  JSON.stringify(
    {
...
say(`\nWrote chisedata/chise-submax-phase3-components.json (${components.length} component records).`);

writeFileSync(join(OUT_DIR, "chise-submax-phase3-prep-log.md"), log.join("\n") + "\n");
say(`Wrote chisedata/chise-submax-phase3-prep-log.md.`);

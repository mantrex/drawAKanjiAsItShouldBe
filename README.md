...
**Why it's called KVG-KRAD.** The colouring model started from KRADFILE and was then adapted to handle the anomalies between KRADFILE and KanjiVG. The main one: KRADFILE lists atomic components (`一`, `丨`, `丿`, `二`, `十`, `土`, ...) that KanjiVG leaves as raw ungrouped strokes, so a walk driven only by KRADFILE's names would fold those strokes into an unrelated neighbouring block — in 右 the diagonal and the horizontal bar would end up inside `口`'s block. `src/kradExtendedData.json` (generated offline by `npm run build-krad-extended`) lists, per affected kanji, the stroke groups that carry such a component; they become blocks of their own named after the component (右 → `丿`, `一`, `口`). Only unambiguous matches are recorded — the component's stroke-type signature is learned from KanjiVG's own tagged instances of it — so an unresolved case keeps the plain KRADFILE behaviour and no label is ever invented.
